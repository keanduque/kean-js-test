...
const poll = {
  ...
  registerNewAnswer() {
    const strOption = this.options + ""; // this.options.join("\n")
    const option = strOption.replace(/,/g, "\n");
    const optQuestion = `${this.question}\n${option}\n(Write option number)`;
    const poll = Number(prompt(optQuestion));

    if (poll < this.answers.length) {
      this.answers[poll] += 1;
      this.displayResults();
    ...
    // OR this short circuiting
    //typeof poll === 'number' &&  poll < this.answers.length && this.answers[poll]++;
  },
};
...
btnPoll.addEventListener("click", newAnswer.bind(poll));
